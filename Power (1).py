...
df = pd.DataFrame(columns=["model_name", "file_name", "num_inferences", "total_time (s)", "load_time (s)", "alloc_time (s)", "inference_time (s)", "average_latency (s)"])
...
df = df.append({"model_name": model_name, "file_name": filename, "num_inferences": num_infer, "total_time (s)": totalTime, "load_time (s)": modelLoadTime, "alloc_time (s)": allocationTime, "inference_time (s)": inferenceTime, "average_latency (s)": average_latency}, ignore_index=True)

# Save DataFrame to CSV
df.to_csv(model_name + str(inference_time) + "_power_timeline.csv", index=False)
